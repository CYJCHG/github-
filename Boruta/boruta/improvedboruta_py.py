...
class ImprovedBorutaPy(BaseEstimator, TransformerMixin):
    """
    ...
    - Scikit-learn like interface:
        Use ImprovesBorutaPy just like any other scikit learner: fit, fit_transform and
    ...
    - Ranking of features:
        After fitting ImprovesBorutaPy it provides the user with ranking of features.
    ...
    from boruta import ImprovesBorutaPy
    
    # load X and y
    # NOTE ImprovesBorutaPy accepts numpy arrays only, hence the .values attribute
    ...
    feat_selector = ImprovesBorutaPy(rf, n_estimators='auto', verbose=2, random_state=1)
    ...
    def __init__(self, estimator, n_estimators=1000, perc=100, alpha=0.05,
                 two_step=True, max_iter=100, random_state=None, verbose=0, pos=1):
    ...
    def _get_imp(self, X, y):
        ...
        try:
            ...
            raise ValueError('Only methods with feature_importance_ attribute '
                             'are currently supported in ImprovesBorutaPy.')
        return imp
    ...
    def _add_shadows_get_imps(self, X, y, dec_reg):
        ...
        x_sha = np.copy(x_cur)
        n_sample,n_feat = x_cur.shape
        x_sha_2 = x_sha[int(n_sample*self.pos):,]
        x_sha = x_sha[0:int(n_sample*self.pos),]
        # make sure there's at least 5 columns in the shadow matrix for
        while (x_sha.shape[1] < 5):
            x_sha = np.hstack((x_sha, x_sha))
        # shuffle xSha
        x_sha2 = np.apply_along_axis(self._get_shuffle, 0, x_sha)
        # get importance of the merged matrix
        x_sha_com = np.vstack((x_sha2,x_sha_2))
        r = np.random.permutation(len(x_sha_com))
        x_sha_sha = x_sha_com[r, :] 
        x_com = np.hstack((x_cur,x_sha_sha))
        imp = self._get_imp(x_com, y)
    ...
    def _check_params(self, X, y):
        ...
        if self.pos < 0 or self.pos > 1:
            raise ValueError('pos should be between 0 and 1.')
    ...
    def _print_results(self, dec_reg, _iter, flag):
        ...
        # Boruta finished running and tentatives have been filtered
            ...
            content = map(str, [n_iter, n_confirmed, n_tentative, n_rejected])
            result = '\n'.join([x[0] + '\t' + x[1] for x in zip(cols, content)])
            output = "\n\nImprovesBorutaPy finished running.\n\n" + result
        print(output)
